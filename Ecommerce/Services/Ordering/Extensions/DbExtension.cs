                ...
                try
                {
                    ...
                    var retry = Policy.Handle<SqlException>()
                        .WaitAndRetry(
                            ...
                            {
                                logger.LogWarning(exception, "[{Prefix}] Exception {ExceptionType} with message {Message} detected on attempt {RetryAttempt} of {RetryCount}", nameof(TContext), exception.GetType().Name, exception.Message, retry, 5);
                            }
                        );
